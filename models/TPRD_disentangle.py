...
class TPRD_disentangle(nn.Module):
    def __init__(self, clip_model, cfg):
        ...
        if 'ViT-B/16' == cfg.clip_model:
            feature_width = 512
        elif 'RN50' == cfg.clip_model:
            feature_width = 1024
        else:
            raise NotImplementedError

        for i in range(len(self.region_prompts)):
            setattr(self,"cat_head%d" %i, ChannelAttention(feature_width))
            setattr(self, "cross_head%d" % i, CrossAttention(feature_width))

        if cfg.onehot:
            self.fc = nn.Sequential(nn.BatchNorm1d(feature_width),nn.ReLU(inplace=True),nn.Linear(feature_width, len(self.cfg.expression_prompts)))
    ...
    def forward(self, image):
        ...
        text_region_features = self.region_embs
        text_region_features = text_region_features / text_region_features.norm(dim=-1, keepdim=True)

        ################  Multi-region  ##############
        heads = []
        for i in range(len(self.region_prompts)):
            disentanle_proj = image_features @ text_region_features[i, :].t().unsqueeze(dim=1) * text_region_features[i,:].unsqueeze(dim=0).repeat(image_features.size(0), 1)
            head = getattr(self, "cat_head%d" % i)(disentanle_proj)
            heads.append(getattr(self, "cross_head%d" % i)(head,image_features))
        heads = torch.stack(heads).permute([1, 0, 2])
        output_expression = heads.sum(dim=1)
        if self.cfg.onehot:
            return self.fc(output_expression),None
        output_expression = output_expression / output_expression.norm(dim=-1, keepdim=True)
        output_expression = output_expression @ text_expression_features.t() * self.logit_scale.exp()
...
class ChannelAttention(nn.Module):
    def __init__(self, input_width):
        ...
        self.attention = nn.Sequential(
            nn.Linear(input_width,input_width//16),
            nn.BatchNorm1d(input_width//16),
            nn.ReLU(inplace=True),
            nn.Linear(input_width//16, input_width),
            nn.Sigmoid()
            # nn.ReLU(inplace=True)
        )
        # self.attention = nn.Sequential(
